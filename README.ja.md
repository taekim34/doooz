...
DOOOZはSupabase + Vercelの無料tierだけで十分に運用できます。画像/ファイルのアップロードがなく、テキストデータのみ保存するため、リソース使用量は非常に少ないです。

### 何家族まで使えますか？

1家族 = 親1名 + 子ども2〜3名、1日のタスクチェック10〜15件の場合:

| リソース | 無料上限 | 家族あたり/月 | 最大家族数 |
|---------|---------|------------|----------|
| Supabase帯域幅 | 5GB/月 | ~6MB | ~800家族 |
| Supabase DB容量 | 500MB | ~230KB/月（累積） | ~180家族（1年基準） |
| Vercel帯域幅 | 100GB/月 | ~5MB | ~18,000家族 |

- **1家族専用**なら無料上限の1〜2%しか使いません。
- **100〜200家族**が1年間使っても無料tier内で運用可能です。
...
長期運用ではDB容量（500MB）が最初に到達する可能性があります:
- 古いタスクインスタンス（完了済みの過去記録）をクリーンアップ
- Supabase Proプランへアップグレード（$25/月、8GB DB）
