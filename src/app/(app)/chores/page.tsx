...
import { ChoreCheckbox } from "./_checkbox";
...
export default async function ChoresPage() {
  ...
  const upcomingAll = ((choresRes.data ?? []) as Chore[]);
  const past = ((overdueOrPast ?? []) as Chore[]);
  const all: Chore[] = [...upcomingAll, ...past];
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">{t("chores.title", locale)}</h1>
          <Link href="/chores/history" className="text-sm text-primary underline">
            {t("chores.history", locale)}
          </Link>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              {t("chores.today", locale)} · {today}{" "}
              <span className="text-sm font-normal text-muted-foreground">
                {todayDone}/{todayAll.length} {t("chores.completed", locale)}
              </span>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            {todayAll.length === 0 && (
              <p className="text-sm text-muted-foreground">{t("chores.no_today", locale)}</p>
            )}
            {todayAll.map((c) => (
              <ChoreCheckbox
                key={c.id}
        ...
        {upcoming.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle>{t("chores.upcoming", locale)}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {upcoming.map((c) => {
                ...
                return (
                  <ChoreCheckbox
                    ...
                    status={c.status}
                    readOnly
                    trailing={d > 0 ? `D-${d}` : t("chores.history_today", locale)}
                  />
                );
              })}
            </CardContent>
        ...
        {overdue.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-red-700">{t("chores.missed", locale)}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {overdue.map((c) => (
                <ChoreCheckbox
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("chores.beg_title", locale)}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <p className="text-xs text-muted-foreground">
              {t("chores.beg_desc", locale)}
            </p>
            <BegForm />
            {myRequested.length > 0 && (
              <div className="space-y-1 pt-2">
                <p className="text-xs text-muted-foreground">{t("chores.beg_waiting", locale)}</p>
                {myRequested.map((c) => (
    ...
    <div className="mx-auto max-w-2xl space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 text-sm">
          <Link href="/chores/history" className="text-primary underline">
            {t("chores.history", locale)}
          </Link>
          <Link href="/chores/manage" className="text-primary underline">
            {t("chores.manage", locale)}
          </Link>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {today} · {t("chores.completed", locale)} {doneCount}/{totalToday}
          </CardTitle>
        </CardHeader>
      </Card>
      ...
      {requested.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-yellow-600">{t("chores.beg_requests", locale)} ({requested.length})</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {requested.map((c) => (
          ...
          <CardHeader>
            <CardTitle className="text-base">
              ...
              <span className="text-xs font-normal text-muted-foreground">
                · {t("chores.history_today", locale)} {doneToday.length}/{todayList.length + doneToday.length}
                {overdue.length > 0 ? ` · ${t("chores.missed", locale)} ${overdue.length}` : ""}
              </span>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            {todayList.length === 0 && overdue.length === 0 && doneToday.length === 0 && (
              <p className="text-sm text-muted-foreground">{t("chores.none", locale)}</p>
            )}
            {todayList.map((c) => (
              <ChoreCheckbox
                key={c.id}
