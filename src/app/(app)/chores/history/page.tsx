...
export default async function ChoreHistoryPage({ searchParams }: Props) {
  ...
  const grouped = rows.reduce<Record<string, typeof rows>>((acc, r) => {
    const key = isParent && !selectedChildId
      ? (childNameMap.get(r.assignee_id) ?? t("chores.filter_other", locale))
      : "items";
    (acc[key] ??= []).push(r);
    return acc;
  }, {});
  ...
  // Children can only modify today's chores; parents can modify any date.
  const childReadOnly = !isParent && isPastDate;

  return (
    <div className="mx-auto max-w-2xl space-y-4">
      <BackButton fallback="/chores" />
      <h1 className="text-2xl font-bold">{t("chores.history_title", locale)}</h1>
      ...
      <div className="flex items-center justify-between text-sm">
        <a
          href={`/chores/history?date=${prevDate}${selectedChildId ? `&child=${selectedChildId}` : ""}`}
          className="rounded border px-3 py-1 hover:bg-muted"
        >
          {t("chores.history_prev", locale)}
        </a>
        <span className="font-medium">{selectedDate}</span>
        {canGoNext ? (
          <a
            href={`/chores/history?date=${nextDate}${selectedChildId ? `&child=${selectedChildId}` : ""}`}
            className="rounded border px-3 py-1 hover:bg-muted"
          >
            {t("chores.history_next", locale)}
          </a>
        ) : (
          <span className="rounded border px-3 py-1 text-muted-foreground">{t("chores.history_today", locale)}</span>
        )}
      </div>
