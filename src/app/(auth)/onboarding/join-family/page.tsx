...
async function joinAction(formData: FormData) {
  ...
  if (!familyName || !displayName) redirect(`/onboarding/join-family?error=${encodeURIComponent(t("auth.error_input", locale))}`);
  ...
  const { data: fam } = await admin
    ...
    .eq("name", familyName)
    .maybeSingle();
  if (!fam) redirect(`/onboarding/join-family?error=${encodeURIComponent(t("auth.error_family_not_found", locale))}`);
  ...
  if (uerr) redirect(`/onboarding/join-family?error=${encodeURIComponent(uerr.message)}`);
